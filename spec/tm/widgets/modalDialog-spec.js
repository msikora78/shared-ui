...
if (isRequire) {
        ...
        describe('tm.widgets.modalDialog', function() {
            for (var version in jquery) {
                if (jquery.hasOwnProperty(version)) {
                    ...
                    runTest(jquery[version], Util, false, ModalDialog);
                    runTest(jquery[version], Util, true, ModalDialog);
                }
            }
        });
    });
    ...
    describe('tm.widgets.modalDialog', function() {
        runTest($, tm.widgets.util, false);
        runTest($, tm.widgets.util, true);
    });
}

function runTest($, Util, isLarge, ModalDialog) {
    describe(isLarge ? 'large with jquery v' + $.fn.jquery : 'with jquery v' + $.fn.jquery, function() {
